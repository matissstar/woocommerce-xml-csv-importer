...
class WC_XML_CSV_AI_Import_Processor {
    ...
    private function process_hybrid($value, $config, $product_data = array()) {
        if (defined('WP_DEBUG') && WP_DEBUG) { error_log("★★★ HYBRID MODE CALLED: value={$value}, config=" . json_encode($config)); }
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) { error_log("★★★ HYBRID: AI Prompt={$ai_prompt}, Provider={$ai_provider}"); }
        
        if (!empty($ai_prompt)) {
            if (defined('WP_DEBUG') && WP_DEBUG) { error_log("★★★ HYBRID: Calling AI processing..."); }
            $context = $this->build_product_context($product_data);
            $value = $this->ai_providers->process_field($value, $ai_prompt, array('provider' => $ai_provider), $context);
            if (defined('WP_DEBUG') && WP_DEBUG) { error_log("★★★ HYBRID: AI result={$value}"); }
        }
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) { error_log("★★★ HYBRID: PHP Formula={$php_formula}"); }
        
        if (!empty($php_formula)) {
            if (defined('WP_DEBUG') && WP_DEBUG) { error_log("★★★ HYBRID: Executing PHP formula..."); }
            $value = $this->execute_php_formula($php_formula, $value, $product_data);
            if (defined('WP_DEBUG') && WP_DEBUG) { error_log("★★★ HYBRID: PHP result={$value}"); }
        }

        if (defined('WP_DEBUG') && WP_DEBUG) { error_log("★★★ HYBRID: Final value={$value}"); }
        return $value;
    }
    ...
    private function execute_php_formula($formula, $value, $product_data = array()) {
        if (defined('WP_DEBUG') && WP_DEBUG) { error_log('[PHP_FORMULA_EXEC] Starting execution'); }
        if (defined('WP_DEBUG') && WP_DEBUG) { error_log('[PHP_FORMULA_EXEC] Formula: ' . substr($formula, 0, 200) . '...'); }
        ...
        try {
            ...
            if (defined('WP_DEBUG') && WP_DEBUG) { error_log('★★★ PHP_FORMULA: About to call safe_eval'); }
            if (defined('WP_DEBUG') && WP_DEBUG) { error_log('★★★ PHP_FORMULA: Formula = ' . $formula); }
            if (defined('WP_DEBUG') && WP_DEBUG) { error_log('★★★ PHP_FORMULA: Variables count = ' . count($variables)); }
            if (defined('WP_DEBUG') && WP_DEBUG) { error_log('★★★ PHP_FORMULA: Value to process = ' . $value); }
            ...
            if (defined('WP_DEBUG') && WP_DEBUG) { error_log('★★★ PHP_FORMULA: safe_eval returned = ' . (is_string($result) ? $result : gettype($result))); }
    ...
    private function safe_eval($code, $variables = array(), $value_param = null) {
        ...
        try {
            ...
            if (defined('WP_DEBUG') && WP_DEBUG) { error_log('★★★ SAFE_EVAL: Normalized formula: ' . $formula_code); }
            ...
            if (defined('WP_DEBUG') && WP_DEBUG) { error_log('★★★ SAFE_EVAL: About to execute: ' . $eval_code); }
            if (defined('WP_DEBUG') && WP_DEBUG) { error_log('★★★ SAFE_EVAL: $value before exec = ' . (isset($value) ? $value : 'NOT SET')); }
            
            // Execute eval with return statement
            try {
                $result = eval('return ' . $eval_code . ';');
                if (defined('WP_DEBUG') && WP_DEBUG) { error_log('★★★ SAFE_EVAL: Result after exec = ' . (is_string($result) ? $result : gettype($result) . ' - ' . var_export($result, true))); }
            } catch (\Throwable $eval_error) {
                if (defined('WP_DEBUG') && WP_DEBUG) { error_log('★★★ SAFE_EVAL: EVAL ERROR: ' . $eval_error->getMessage()); }
                throw $eval_error;
            }
