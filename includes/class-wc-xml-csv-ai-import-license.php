...
class WC_XML_CSV_AI_Import_License {
    ...
    public static function validate($license_key = null) {
        ...
        if (empty($license_key)) {
            return array(
                'valid' => false,
                'message' => __('No license key provided.', 'bootflow-product-importer'),
                'features' => array(),
                'expires_at' => null,
            );
        }
    ...
    private static function api_request($license_key) {
        ...
        if (is_wp_error($response)) {
            return array(
                'valid' => false,
                'message' => __('Could not connect to license server. Please try again later.', 'bootflow-product-importer'),
                'features' => array(),
                'expires_at' => null,
        ...
        if ($code !== 200 || !is_array($data)) {
            return array(
                'valid' => false,
                'message' => isset($data['message']) ? $data['message'] : __('Invalid license key.', 'bootflow-product-importer'),
                'features' => array(),
                'expires_at' => null,
            );
        }
    ...
    public static function activate_license($license_key) {
        ...
        return array(
            ...
            'message' => sprintf(
                __('License activated successfully! Your plan: %s', 'bootflow-product-importer'),
    ...
    public static function deactivate_license() {
        ...
        return array(
            'success' => true,
            'message' => __('License deactivated.', 'bootflow-product-importer'),
        );
    }
}
