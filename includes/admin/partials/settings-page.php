...
if (isset($_POST['submit']) && check_admin_referer('wc_xml_csv_ai_import_settings')) {
    ...
    $ai_settings = array(
        ...
        'claude_model' => sanitize_text_field(wp_unslash($_POST['claude_model'] ?? 'claude-3-sonnet-20240229')),
    ...
    echo '<div class="notice notice-success"><p>' . __('Settings saved successfully!', 'bootflow-woocommerce-xml-csv-importer') . '</p></div>';
}
...
<div class="wrap">
    <h1><?php _e('XML/CSV AI Smart Import - Settings', 'bootflow-woocommerce-xml-csv-importer'); ?></h1>
    
    <?php $current_tier = WC_XML_CSV_AI_Import_License::get_tier(); ?>
    <nav class="nav-tab-wrapper">
        <a href="#license" class="nav-tab nav-tab-active"><?php _e('License', 'bootflow-woocommerce-xml-csv-importer'); ?></a>
        <?php if ($current_tier === 'pro'): ?>
        <a href="#ai-providers" class="nav-tab"><?php _e('AI Providers', 'bootflow-woocommerce-xml-csv-importer'); ?></a>
        <?php endif; ?>
        <a href="#performance" class="nav-tab"><?php _e('Performance', 'bootflow-woocommerce-xml-csv-importer'); ?></a>
        <a href="#import" class="nav-tab"><?php _e('Import', 'bootflow-woocommerce-xml-csv-importer'); ?></a>
        <a href="#scheduling" class="nav-tab"><?php _e('Scheduling', 'bootflow-woocommerce-xml-csv-importer'); ?></a>
        <a href="#files" class="nav-tab"><?php _e('Files', 'bootflow-woocommerce-xml-csv-importer'); ?></a>
        <a href="#logging" class="nav-tab"><?php _e('Logging', 'bootflow-woocommerce-xml-csv-importer'); ?></a>
        <a href="#security" class="nav-tab"><?php _e('Security', 'bootflow-woocommerce-xml-csv-importer'); ?></a>
    </nav>
    ...
    <form method="post" action="">
        ...
        <div id="license" class="tab-content active">
            <h2><?php _e('License Management', 'bootflow-woocommerce-xml-csv-importer'); ?></h2>
                ...
                <div style="display: flex; align-items: center; gap: 20px; flex-wrap: wrap;">
                    ...
                    <div style="flex: 1;">
                        <h3 style="margin: 0 0 5px 0; color: white; font-size: 24px; text-transform: uppercase; letter-spacing: 1px;">
                            <?php echo esc_html(strtoupper($current_tier)); ?> <?php _e('Edition', 'bootflow-woocommerce-xml-csv-importer'); ?>
                        </h3>
                        <p style="margin: 0; font-size: 14px; color: rgba(255,255,255,0.9);">
                            <?php if ($current_tier === 'free'): ?>
                                <?php _e('Full manual import tool. Upgrade to PRO for automation, templates, and AI features!', 'bootflow-woocommerce-xml-csv-importer'); ?>
                            <?php else: ?>
                                <?php _e('All features unlocked including AI Auto-Mapping, templates, and scheduled imports!', 'bootflow-woocommerce-xml-csv-importer'); ?>
                            <?php endif; ?>
                        </p>
                    </div>
                    <?php if ($license_status === 'active' && !empty($license_key)): ?>
                        <div style="text-align: right;">
                            <span style="background: rgba(255,255,255,0.2); padding: 5px 15px; border-radius: 20px; font-size: 12px;">
                                ✓ <?php _e('License Active', 'bootflow-woocommerce-xml-csv-importer'); ?>
                            </span>
                        </div>
                    <?php endif; ?>
                </div>
            ...
            <div style="background: #fff; border: 1px solid #ddd; border-radius: 8px; padding: 25px; margin-bottom: 25px;">
                <h3 style="margin: 0 0 15px 0; display: flex; align-items: center; gap: 10px;">
                    ...
                    <?php _e('Activate License', 'bootflow-woocommerce-xml-csv-importer'); ?>
                </h3>
                
                <p class="description" style="margin-bottom: 15px;">
                    <?php _e('Enter your license key to unlock PRO features. Purchase a license at', 'bootflow-woocommerce-xml-csv-importer'); ?>
                ...
                <div style="display: flex; gap: 10px; align-items: flex-start; flex-wrap: wrap;">
                    <div style="flex: 1; min-width: 300px;">
                        <input type="text" 
                               ...
                               placeholder="<?php esc_attr_e('Enter your license key (e.g., PRO-XXXX-XXXX-XXXX)', 'bootflow-woocommerce-xml-csv-importer'); ?>" 
                               class="regular-text" 
                    ...
                    <button type="button" id="btn-activate-license" class="button button-primary" style="height: 40px; padding: 0 25px;">
                        ...
                        <?php _e('Activate', 'bootflow-woocommerce-xml-csv-importer'); ?>
                    </button>
                    <?php if ($license_status === 'active' && !empty($license_key)): ?>
                        <button type="button" id="btn-deactivate-license" class="button" style="height: 40px; padding: 0 25px; color: #dc3545; border-color: #dc3545;">
                            ...
                            <?php _e('Deactivate', 'bootflow-woocommerce-xml-csv-importer'); ?>
                        </button>
                    <?php endif; ?>
                </div>
            ...
            <div style="background: #fff; border: 1px solid #ddd; border-radius: 8px; padding: 25px;">
                <h3 style="margin: 0 0 20px 0; display: flex; align-items: center; gap: 10px;">
                    ...
                    <?php _e('Feature Comparison', 'bootflow-woocommerce-xml-csv-importer'); ?>
                </h3>
                
                <table class="widefat striped" style="margin: 0;">
                    <thead>
                        <tr>
                            <th style="width: 50%;"><?php _e('Feature', 'bootflow-woocommerce-xml-csv-importer'); ?></th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><?php _e('Import XML & CSV feeds', 'bootflow-woocommerce-xml-csv-importer'); ?></td>
                            <td style="text-align: center; color: #28a745;">✓</td>
                            <td style="text-align: center; color: #28a745;">✓</td>
                        </tr>
                        <tr>
                            <td><?php _e('Manual field mapping', 'bootflow-woocommerce-xml-csv-importer'); ?></td>
                            <td style="text-align: center; color: #28a745;">✓</td>
                            <td style="text-align: center; color: #28a745;">✓</td>
                        </tr>
                        <tr>
                            <td><?php _e('Simple & variable products', 'bootflow-woocommerce-xml-csv-importer'); ?></td>
                            <td style="text-align: center; color: #28a745;">✓</td>
                            <td style="text-align: center; color: #28a745;">✓</td>
                        </tr>
                        <tr>
                            <td><?php _e('Attributes & variations', 'bootflow-woocommerce-xml-csv-importer'); ?></td>
                            <td style="text-align: center; color: #28a745;">✓</td>
                            <td style="text-align: center; color: #28a745;">✓</td>
                        </tr>
                        <tr>
                            <td><?php _e('Manual imports', 'bootflow-woocommerce-xml-csv-importer'); ?></td>
                            <td style="text-align: center; color: #28a745;">✓</td>
                            <td style="text-align: center; color: #28a745;">✓</td>
                        </tr>
                        <tr>
                            <td><?php _e('Unlimited products', 'bootflow-woocommerce-xml-csv-importer'); ?></td>
                            <td style="text-align: center; color: #28a745;">✓</td>
                            <td style="text-align: center; color: #28a745;">✓</td>
                        </tr>
                        <tr>
                            <td><?php _e('Local file uploads', 'bootflow-woocommerce-xml-csv-importer'); ?></td>
                            <td style="text-align: center; color: #28a745;">✓</td>
                            <td style="text-align: center; color: #28a745;">✓</td>
                        </tr>
                        <tr>
                            <td><?php _e('Update existing products', 'bootflow-woocommerce-xml-csv-importer'); ?></td>
                            <td style="text-align: center; color: #28a745;">✓</td>
                            <td style="text-align: center; color: #28a745;">✓</td>
                        </tr>
                        ...
                        <tr style="background: #f0f9f9;">
                            <td><strong><?php _e('Select which fields to update', 'bootflow-woocommerce-xml-csv-importer'); ?></strong></td>
                            <td style="text-align: center; color: #dc3545;">✗</td>
                            <td style="text-align: center; color: #28a745;">✓</td>
                        </tr>
                        <tr style="background: #f0f9f9;">
                            <td><strong><?php _e('Update only price and stock', 'bootflow-woocommerce-xml-csv-importer'); ?></strong></td>
                            <td style="text-align: center; color: #dc3545;">✗</td>
                            <td style="text-align: center; color: #28a745;">✓</td>
                        </tr>
                        <tr style="background: #f0f9f9;">
                            <td><strong><?php _e('Automatic field mapping', 'bootflow-woocommerce-xml-csv-importer'); ?></strong></td>
                            <td style="text-align: center; color: #dc3545;">✗</td>
                            <td style="text-align: center; color: #28a745;">✓</td>
                        </tr>
                        <tr style="background: #f0f9f9;">
                            <td><strong><?php _e('Scheduled imports (cron)', 'bootflow-woocommerce-xml-csv-importer'); ?></strong></td>
                            <td style="text-align: center; color: #dc3545;">✗</td>
                            <td style="text-align: center; color: #28a745;">✓</td>
                        </tr>
                        <tr style="background: #f0f9f9;">
                            <td><strong><?php _e('Import from remote URLs', 'bootflow-woocommerce-xml-csv-importer'); ?></strong></td>
                            <td style="text-align: center; color: #dc3545;">✗</td>
                            <td style="text-align: center; color: #28a745;">✓</td>
                        </tr>
                        <tr style="background: #f0f9f9;">
                            <td><strong><?php _e('Advanced update rules', 'bootflow-woocommerce-xml-csv-importer'); ?></strong></td>
                            <td style="text-align: center; color: #dc3545;">✗</td>
                            <td style="text-align: center; color: #28a745;">✓</td>
                        </tr>
                        <tr style="background: #f0f9f9;">
                            <td><strong><?php _e('Conditional logic', 'bootflow-woocommerce-xml-csv-importer'); ?></strong></td>
                            <td style="text-align: center; color: #dc3545;">✗</td>
                            <td style="text-align: center; color: #28a745;">✓</td>
                        </tr>
                        <tr style="background: #f0f9f9;">
                            <td><strong><?php _e('Import templates', 'bootflow-woocommerce-xml-csv-importer'); ?></strong></td>
                            <td style="text-align: center; color: #dc3545;">✗</td>
                            <td style="text-align: center; color: #28a745;">✓</td>
                        </tr>
                        <tr style="background: #f0f9f9;">
                            <td><strong><?php _e('Logs & error reporting', 'bootflow-woocommerce-xml-csv-importer'); ?></strong></td>
                            <td style="text-align: center; color: #dc3545;">✗</td>
                            <td style="text-align: center; color: #28a745;">✓</td>
                        </tr>
                        <tr style="background: #e8f5f5;">
                            <td><strong>🤖 <?php _e('AI-assisted mapping & transformation', 'bootflow-woocommerce-xml-csv-importer'); ?></strong></td>
                            <td style="text-align: center; color: #dc3545;">✗</td>
                            <td style="text-align: center; color: #28a745;">✓</td>
                        </tr>
                        <tr style="background: #e8f5f5;">
                            <td><strong>🌍 <?php _e('Translate product data with AI', 'bootflow-woocommerce-xml-csv-importer'); ?></strong></td>
                            <td style="text-align: center; color: #dc3545;">✗</td>
                            <td style="text-align: center; color: #28a745;">✓</td>
                        </tr>
                        <tr style="background: #e8f5f5;">
                            <td><strong>💬 <?php _e('Custom AI prompts per field', 'bootflow-woocommerce-xml-csv-importer'); ?></strong></td>
                            <td style="text-align: center; color: #dc3545;">✗</td>
                            <td style="text-align: center; color: #28a745;">✓</td>
                        </tr>
                        <tr style="background: #f0f9f9;">
                            <td><strong>🎯 <?php _e('Priority support', 'bootflow-woocommerce-xml-csv-importer'); ?></strong></td>
                            <td style="text-align: center; color: #dc3545;">✗</td>
                            <td style="text-align: center; color: #28a745;">✓</td>
                        </tr>
                    </tbody>
        ...
        <div id="ai-providers" class="tab-content">
            <h2><?php _e('AI Provider Configuration', 'bootflow-woocommerce-xml-csv-importer'); ?></h2>
            <p class="description"><?php _e('Configure API keys and settings for AI providers used in smart field processing.', 'bootflow-woocommerce-xml-csv-importer'); ?></p>
            
            <table class="form-table">
                <!-- OpenAI -->
                <tr>
                    <th scope="row"><?php _e('OpenAI Settings', 'bootflow-woocommerce-xml-csv-importer'); ?></th>
                    <td>
                        <fieldset>
                            <label for="openai_api_key"><?php _e('API Key:', 'bootflow-woocommerce-xml-csv-importer'); ?></label><br>
                            ...
                            <button type="button" class="toggle-password button button-small"><?php _e('Show', 'bootflow-woocommerce-xml-csv-importer'); ?></button><br><br>
                            
                            <label for="openai_model"><?php _e('Model:', 'bootflow-woocommerce-xml-csv-importer'); ?></label><br>
                ...
                <!-- Google Gemini -->
                <tr>
                    <th scope="row"><?php _e('Google Gemini Settings', 'bootflow-woocommerce-xml-csv-importer'); ?></th>
                    <td>
                        <fieldset>
                            <label for="gemini_api_key"><?php _e('API Key:', 'bootflow-woocommerce-xml-csv-importer'); ?></label><br>
                            ...
                            <label for="gemini_model"><?php _e('Model:', 'bootflow-woocommerce-xml-csv-importer'); ?></label><br>
                ...
                <!-- Anthropic Claude -->
                <tr>
                    <th scope="row"><?php _e('Anthropic Claude Settings', 'bootflow-woocommerce-xml-csv-importer'); ?></th>
                    <td>
                        <fieldset>
                            <label for="claude_api_key"><?php _e('API Key:', 'bootflow-woocommerce-xml-csv-importer'); ?></label><br>
                            ...
                            <label for="claude_model"><?php _e('Model:', 'bootflow-woocommerce-xml-csv-importer'); ?></label><br>
                ...
                <!-- xAI Grok -->
                <tr>
                    <th scope="row"><?php _e('xAI Grok Settings', 'bootflow-woocommerce-xml-csv-importer'); ?></th>
                    <td>
                        <fieldset>
                            <label for="grok_api_key"><?php _e('API Key:', 'bootflow-woocommerce-xml-csv-importer'); ?></label><br>
                            ...
                            <label for="grok_model"><?php _e('Model:', 'bootflow-woocommerce-xml-csv-importer'); ?></label><br>
                ...
                <!-- GitHub Copilot -->
                <tr>
                    <th scope="row"><?php _e('GitHub Copilot Settings', 'bootflow-woocommerce-xml-csv-importer'); ?></th>
                    <td>
                        <fieldset>
                            <label for="copilot_api_key"><?php _e('API Key:', 'bootflow-woocommerce-xml-csv-importer'); ?></label><br>
                            ...
                            <label for="copilot_model"><?php _e('Model:', 'bootflow-woocommerce-xml-csv-importer'); ?></label><br>
                ...
                <!-- General AI Settings -->
                <tr>
                    <th scope="row"><?php _e('General AI Settings', 'bootflow-woocommerce-xml-csv-importer'); ?></th>
                    <td>
                        <fieldset>
                            <label for="default_provider"><?php _e('Default Provider:', 'bootflow-woocommerce-xml-csv-importer'); ?></label><br>
                            ...
                            <label>
                                ...
                                <?php _e('Enable Provider Fallback', 'bootflow-woocommerce-xml-csv-importer'); ?>
                            </label><br><br>
                            
                            <label for="ai_timeout"><?php _e('API Timeout (seconds):', 'bootflow-woocommerce-xml-csv-importer'); ?></label><br>
                            <input type="number" id="ai_timeout" name="ai_timeout" value="<?php echo esc_attr($ai_settings['ai_timeout'] ?? 30); ?>" min="10" max="300" /><br><br>
                            
                            <label for="ai_max_retries"><?php _e('Max Retries:', 'bootflow-woocommerce-xml-csv-importer'); ?></label><br>
                            ...
                            <label>
                                ...
                                <?php _e('Enable AI Response Caching', 'bootflow-woocommerce-xml-csv-importer'); ?>
                            </label><br><br>
                            
                            <label for="ai_cache_ttl"><?php _e('Cache TTL (seconds):', 'bootflow-woocommerce-xml-csv-importer'); ?></label><br>
        ...
        <div id="performance" class="tab-content">
            <h2><?php _e('Performance Settings', 'bootflow-woocommerce-xml-csv-importer'); ?></h2>
            <p class="description"><?php _e('Configure performance-related settings to optimize import speed and memory usage.', 'bootflow-woocommerce-xml-csv-importer'); ?></p>
            
            <table class="form-table">
                <tr>
                    <th scope="row"><?php _e('Memory & Processing', 'bootflow-woocommerce-xml-csv-importer'); ?></th>
                    <td>
                        <fieldset>
                            <label for="batch_size"><?php _e('Batch Size:', 'bootflow-woocommerce-xml-csv-importer'); ?></label><br>
                            <input type="number" id="batch_size" name="batch_size" value="<?php echo esc_attr($performance_settings['batch_size'] ?? 50); ?>" min="1" max="500" />
                            <p class="description"><?php _e('Number of products to process in each batch.', 'bootflow-woocommerce-xml-csv-importer'); ?></p><br>
                            
                            <label for="memory_limit"><?php _e('Memory Limit:', 'bootflow-woocommerce-xml-csv-importer'); ?></label><br>
                            ...
                            <label for="max_execution_time"><?php _e('Max Execution Time (seconds):', 'bootflow-woocommerce-xml-csv-importer'); ?></label><br>
                            ...
                            <label for="chunk_size"><?php _e('File Chunk Size:', 'bootflow-woocommerce-xml-csv-importer'); ?></label><br>
                            ...
                            <p class="description"><?php _e('Number of lines to read from file at once.', 'bootflow-woocommerce-xml-csv-importer'); ?></p>
                        </fieldset>
                    </td>
                </tr>
                
                <tr>
                    <th scope="row"><?php _e('Background Processing', 'bootflow-woocommerce-xml-csv-importer'); ?></th>
                    <td>
                        <fieldset>
                            <label>
                                ...
                                <?php _e('Enable Background Processing (WP-Cron)', 'bootflow-woocommerce-xml-csv-importer'); ?>
                            </label><br><br>
                            
                            <label for="background_batch_size"><?php _e('Background Batch Size:', 'bootflow-woocommerce-xml-csv-importer'); ?></label><br>
                            ...
                            <label for="background_interval"><?php _e('Background Interval (seconds):', 'bootflow-woocommerce-xml-csv-importer'); ?></label><br>
        ...
        <div id="import" class="tab-content">
            <h2><?php _e('Import Settings', 'bootflow-woocommerce-xml-csv-importer'); ?></h2>
            <p class="description"><?php _e('Configure default settings for product imports and data handling.', 'bootflow-woocommerce-xml-csv-importer'); ?></p>
            
            <table class="form-table">
                <tr>
                    <th scope="row"><?php _e('Product Defaults', 'bootflow-woocommerce-xml-csv-importer'); ?></th>
                    <td>
                        <fieldset>
                            <label for="default_product_status"><?php _e('Default Product Status:', 'bootflow-woocommerce-xml-csv-importer'); ?></label><br>
                            ...
                            <label for="duplicate_handling"><?php _e('Duplicate Handling:', 'bootflow-woocommerce-xml-csv-importer'); ?></label><br>
                ...
                <tr>
                    <th scope="row"><?php _e('Media & Images', 'bootflow-woocommerce-xml-csv-importer'); ?></th>
                    <td>
                        <fieldset>
                            <label>
                                <input type="checkbox" name="enable_image_download" value="1" <?php checked($import_settings['enable_image_download'] ?? 0, 1); ?> />
                                <?php _e('Download Images from URLs', 'bootflow-woocommerce-xml-csv-importer'); ?>
                            </label><br><br>
                            
                            <label for="image_timeout"><?php _e('Image Download Timeout (seconds):', 'bootflow-woocommerce-xml-csv-importer'); ?></label><br>
                            ...
                            <label for="max_image_size"><?php _e('Max Image Size (MB):', 'bootflow-woocommerce-xml-csv-importer'); ?></label><br>
        ...
        <div id="scheduling" class="tab-content">
            <h2><?php _e('Scheduled Imports (Cron)', 'bootflow-woocommerce-xml-csv-importer'); ?></h2>
            ...
            <div style="background: #fff; border: 1px solid #c3c4c7; border-radius: 6px; padding: 20px; margin-bottom: 20px;">
                <h3 style="margin-top: 0;">
                    ...
                    <?php _e('Scheduling Method', 'bootflow-woocommerce-xml-csv-importer'); ?>
                </h3>
                
                <table class="form-table" style="margin: 0;">
                    <tr>
                        <th scope="row"><?php _e('Choose Method', 'bootflow-woocommerce-xml-csv-importer'); ?></th>
                        <td>
                            <fieldset>
                                <label style="display: block; margin-bottom: 15px; padding: 15px; border: 2px solid <?php echo $scheduling_method === 'action_scheduler' ? '#0073aa' : '#ddd'; ?>; border-radius: 6px; cursor: pointer; background: <?php echo $scheduling_method === 'action_scheduler' ? '#f0f6fc' : '#fff'; ?>;">
                                    ...
                                    <span style="background: #28a745; color: white; font-size: 11px; padding: 2px 8px; border-radius: 10px; margin-left: 8px;"><?php _e('Recommended', 'bootflow-woocommerce-xml-csv-importer'); ?></span>
                                    <?php if (!$action_scheduler_available): ?>
                                        <span style="background: #dc3545; color: white; font-size: 11px; padding: 2px 8px; border-radius: 10px; margin-left: 8px;"><?php _e('Not Available', 'bootflow-woocommerce-xml-csv-importer'); ?></span>
                                    <?php endif; ?>
                                    <p class="description" style="margin: 8px 0 0 24px;">
                                        <?php _e('Uses WooCommerce Action Scheduler for reliable background processing. Automatically processes all products until complete. Works without server cron configuration.', 'bootflow-woocommerce-xml-csv-importer'); ?>
                                        <br><br>
                                        <strong><?php _e('Pros:', 'bootflow-woocommerce-xml-csv-importer'); ?></strong> <?php _e('No server configuration needed, self-healing, processes full import automatically.', 'bootflow-woocommerce-xml-csv-importer'); ?>
                                        <br>
                                        <strong><?php _e('Cons:', 'bootflow-woocommerce-xml-csv-importer'); ?></strong> <?php _e('Requires some website traffic to trigger (or WP-Cron).', 'bootflow-woocommerce-xml-csv-importer'); ?>
                                    </p>
                                </label>
                                ...
                                <label style="display: block; padding: 15px; border: 2px solid <?php echo $scheduling_method === 'server_cron' ? '#0073aa' : '#ddd'; ?>; border-radius: 6px; cursor: pointer; background: <?php echo $scheduling_method === 'server_cron' ? '#f0f6fc' : '#fff'; ?>;">
                                    ...
                                    <strong><?php _e('Server Cron (URL Trigger)', 'bootflow-woocommerce-xml-csv-importer'); ?></strong>
                                    <p class="description" style="margin: 8px 0 0 24px;">
                                        <?php _e('Use an external server cron job to trigger imports via URL. Processes entire import in one request (with loop until complete).', 'bootflow-woocommerce-xml-csv-importer'); ?>
                                        <br><br>
                                        <strong><?php _e('Pros:', 'bootflow-woocommerce-xml-csv-importer'); ?></strong> <?php _e('100% reliable, does not depend on website traffic.', 'bootflow-woocommerce-xml-csv-importer'); ?>
                                        <br>
                                        <strong><?php _e('Cons:', 'bootflow-woocommerce-xml-csv-importer'); ?></strong> <?php _e('Requires server access to configure crontab.', 'bootflow-woocommerce-xml-csv-importer'); ?>
                                    </p>
                                </label>
                            </fieldset>
                        </td>
            ...
            <div class="cron-setup-instructions" id="server-cron-instructions" style="background: #f8f9fa; border: 1px solid #e2e4e7; border-radius: 6px; padding: 20px; margin-bottom: 20px; <?php echo $scheduling_method !== 'server_cron' ? 'display: none;' : ''; ?>">
                <h3 style="margin-top: 0;">
                    ...
                    <?php _e('Server Cron Setup Instructions', 'bootflow-woocommerce-xml-csv-importer'); ?>
                </h3>
                
                <p><?php _e('Add this cron job to your server to trigger scheduled imports. The import will process ALL products until complete in one request.', 'bootflow-woocommerce-xml-csv-importer'); ?></p>
                
                <h4><?php _e('Add this cron job to your server', 'bootflow-woocommerce-xml-csv-importer'); ?></h4>
                <p class="description"><?php _e('Copy and paste this command into your server\'s crontab (via SSH or your hosting control panel):', 'bootflow-woocommerce-xml-csv-importer'); ?></p>
                ...
                <p class="description"><?php _e('Note: We recommend running every minute. The plugin will only process imports when their scheduled interval has passed.', 'bootflow-woocommerce-xml-csv-importer'); ?></p>
                ...
                <h4 style="margin-top: 20px;"><?php _e('Step 2 (Optional): Disable WP-Cron', 'bootflow-woocommerce-xml-csv-importer'); ?></h4>
                <p class="description"><?php _e('For better reliability, add this to your wp-config.php file:', 'bootflow-woocommerce-xml-csv-importer'); ?></p>
                ...
                <h4 style="margin-top: 20px;"><?php _e('Alternative: WP-CLI', 'bootflow-woocommerce-xml-csv-importer'); ?></h4>
                <div style="background: #1e1e1e; color: #d4d4d4; padding: 15px; border-radius: 4px; font-family: monospace; font-size: 13px; margin: 10px 0;">
            ...
            <table class="form-table">
                <tr>
                    <th scope="row"><?php _e('Cron Secret Key', 'bootflow-woocommerce-xml-csv-importer'); ?></th>
                    <td>
                        ...
                        <button type="button" class="button button-small" onclick="this.previousElementSibling.value = '<?php echo wp_generate_password(32, false); ?>'; alert('New secret generated. Save settings to apply.');">
                            <?php _e('Regenerate', 'bootflow-woocommerce-xml-csv-importer'); ?>
                        </button>
                        <p class="description"><?php _e('This secret key protects your cron endpoint from unauthorized access.', 'bootflow-woocommerce-xml-csv-importer'); ?></p>
                    </td>
                </tr>
                <tr>
                    <th scope="row"><?php _e('Cron URL', 'bootflow-woocommerce-xml-csv-importer'); ?></th>
                    <td>
                        <code style="background: #f0f0f0; padding: 8px 12px; display: block; word-break: break-all;"><?php echo esc_url($cron_url); ?></code>
                        <p class="description"><?php _e('This is the URL that the cron job will call to trigger scheduled imports.', 'bootflow-woocommerce-xml-csv-importer'); ?></p>
                    </td>
                </tr>
            </table>
            ...
            <div style="background: linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%); border: 1px solid #dee2e6; border-radius: 8px; padding: 30px; text-align: center;">
                ...
                <h3 style="margin: 0 0 10px 0; color: #495057;">
                    <?php _e('Scheduled Imports', 'bootflow-woocommerce-xml-csv-importer'); ?>
                ...
                <p style="color: #6c757d; margin-bottom: 20px;">
                    <?php _e('Automate your imports with scheduled cron jobs. Run imports automatically every 15 minutes, hourly, daily, or weekly.', 'bootflow-woocommerce-xml-csv-importer'); ?>
                </p>
                <ul style="text-align: left; display: inline-block; margin: 0 0 20px 0; color: #6c757d;">
                    <li>✓ <?php _e('Set up automatic import schedules', 'bootflow-woocommerce-xml-csv-importer'); ?></li>
                    <li>✓ <?php _e('Import from remote URLs', 'bootflow-woocommerce-xml-csv-importer'); ?></li>
                    <li>✓ <?php _e('Keep products always up-to-date', 'bootflow-woocommerce-xml-csv-importer'); ?></li>
                    <li>✓ <?php _e('No manual intervention required', 'bootflow-woocommerce-xml-csv-importer'); ?></li>
                </ul>
                <br>
        ...
        <div id="files" class="tab-content">
            <h2><?php _e('File Management Settings', 'bootflow-woocommerce-xml-csv-importer'); ?></h2>
            <p class="description"><?php _e('Configure file upload, storage, and management settings.', 'bootflow-woocommerce-xml-csv-importer'); ?></p>
            
            <table class="form-table">
                <tr>
                    <th scope="row"><?php _e('Upload Settings', 'bootflow-woocommerce-xml-csv-importer'); ?></th>
                    <td>
                        <fieldset>
                            <label for="max_file_size"><?php _e('Max File Size (MB):', 'bootflow-woocommerce-xml-csv-importer'); ?></label><br>
                            ...
                            <label><?php _e('Allowed File Types:', 'bootflow-woocommerce-xml-csv-importer'); ?></label><br>
                            <label>
                            ...
                            <label for="upload_directory"><?php _e('Upload Directory:', 'bootflow-woocommerce-xml-csv-importer'); ?></label><br>
                            ...
                            <p class="description"><?php _e('Directory name within wp-content/uploads/', 'bootflow-woocommerce-xml-csv-importer'); ?></p>
                        </fieldset>
                    </td>
                </tr>
                
                <tr>
                    <th scope="row"><?php _e('File Retention', 'bootflow-woocommerce-xml-csv-importer'); ?></th>
                    <td>
                        <fieldset>
                            <label>
                                <input type="checkbox" name="auto_delete_files" value="1" <?php checked($file_settings['auto_delete_files'] ?? 0, 1); ?> />
                                <?php _e('Auto-delete Old Files', 'bootflow-woocommerce-xml-csv-importer'); ?>
                            </label><br><br>
                            
                            <label for="file_retention_days"><?php _e('File Retention (days):', 'bootflow-woocommerce-xml-csv-importer'); ?></label><br>
                            ...
                            <p class="description"><?php _e('Files older than this will be automatically deleted.', 'bootflow-woocommerce-xml-csv-importer'); ?></p>
                        </fieldset>
                    </td>
                </tr>
            </table>
        ...
        <div id="logging" class="tab-content">
            <h2><?php _e('Logging & Notifications', 'bootflow-woocommerce-xml-csv-importer'); ?></h2>
            <p class="description"><?php _e('Configure logging levels and notification settings.', 'bootflow-woocommerce-xml-csv-importer'); ?></p>
            
            <table class="form-table">
                <tr>
                    <th scope="row"><?php _e('Logging Configuration', 'bootflow-woocommerce-xml-csv-importer'); ?></th>
                    <td>
                        <fieldset>
                            <label>
                                <input type="checkbox" name="enable_logging" value="1" <?php checked($logging_settings['enable_logging'] ?? 0, 1); ?> />
                                <?php _e('Enable Logging', 'bootflow-woocommerce-xml-csv-importer'); ?>
                            </label><br><br>
                            
                            <label for="log_level"><?php _e('Log Level:', 'bootflow-woocommerce-xml-csv-importer'); ?></label><br>
                            ...
                            <label for="max_log_entries"><?php _e('Max Log Entries:', 'bootflow-woocommerce-xml-csv-importer'); ?></label><br>
                            ...
                            <label for="log_retention_days"><?php _e('Log Retention (days):', 'bootflow-woocommerce-xml-csv-importer'); ?></label><br>
                ...
                <tr>
                    <th scope="row"><?php _e('Email Notifications', 'bootflow-woocommerce-xml-csv-importer'); ?></th>
                    <td>
                        <fieldset>
                            <label>
                                <input type="checkbox" name="email_notifications" value="1" <?php checked($logging_settings['email_notifications'] ?? 0, 1); ?> />
                                <?php _e('Enable Email Notifications', 'bootflow-woocommerce-xml-csv-importer'); ?>
                            </label><br><br>
                            
                            <label for="notification_email"><?php _e('Notification Email:', 'bootflow-woocommerce-xml-csv-importer'); ?></label><br>
        ...
        <div id="security" class="tab-content">
            <h2><?php _e('Security Settings', 'bootflow-woocommerce-xml-csv-importer'); ?></h2>
            <p class="description"><?php _e('Configure security settings for PHP formula execution and data validation.', 'bootflow-woocommerce-xml-csv-importer'); ?></p>
            
            <table class="form-table">
                <tr>
                    <th scope="row"><?php _e('PHP Formula Security', 'bootflow-woocommerce-xml-csv-importer'); ?></th>
                    <td>
                        <fieldset>
                            <label><?php _e('Allowed PHP Functions:', 'bootflow-woocommerce-xml-csv-importer'); ?></label><br>
                            <?php 
                            ...
                            <label>
                                ...
                                <?php _e('Enable Formula Validation', 'bootflow-woocommerce-xml-csv-importer'); ?>
                            </label><br><br>
                            
                            <label for="max_formula_length"><?php _e('Max Formula Length:', 'bootflow-woocommerce-xml-csv-importer'); ?></label><br>
                ...
                <tr>
                    <th scope="row"><?php _e('Data Sanitization', 'bootflow-woocommerce-xml-csv-importer'); ?></th>
                    <td>
                        <fieldset>
                            <label>
                                <input type="checkbox" name="enable_sanitization" value="1" <?php checked($security_settings['enable_sanitization'] ?? 0, 1); ?> />
                                <?php _e('Enable Advanced Data Sanitization', 'bootflow-woocommerce-xml-csv-importer'); ?>
                            </label>
                            <p class="description"><?php _e('Automatically sanitize imported data to prevent XSS and other security issues.', 'bootflow-woocommerce-xml-csv-importer'); ?></p>
                        </fieldset>
                    </td>
                </tr>
            </table>
...
jQuery(document).ready(function($) {
    ...
    $('.toggle-password').on('click', function() {
        ...
        $(this).text(type === 'password' ? '<?php _e('Show', 'bootflow-woocommerce-xml-csv-importer'); ?>' : '<?php _e('Hide', 'bootflow-woocommerce-xml-csv-importer'); ?>');
    });
